...
class FeatureAnalysisItem(BaseModel):
    ...
    feature_full_score: float = Field(default=0.0, description="特征占比百分比，等于特征有效长度/所属独立权利要求有效长度*100")
    feature_awarded_score: float = Field(default=0.0, description="特征加权贡献分，等于特征得分*特征占比")
...
class FeatureComparisonItem(BaseModel):
    ...
    feature_text: str = Field(..., description="权利要求技术特征原文")
    evidence: str = Field(default="", description="商品文字或图片中的证据引用")
    similarity_score: float = Field(..., description="特征得分，范围0-100，等于本特征命中有效长度/本特征有效长度*100")
    score_band: str = Field(..., description="分值分段标签")
...
class ProductComparisonResult(BaseModel):
    ...
    product_similarity_score: float = Field(default=0.0, description="商品总分，所有特征得分按特征占比加权求和；任一特征明确不相同则归零")
