...
# 任务结果存储：run_id -> {"status": "processing"|"completed"|"failed"|"timeout", "result": ..., "error": ...}
...
async def _run_task_and_store(run_id: str, payload: Dict[str, Any], ctx):
    """在后台执行工作流任务，完成后将结果存入 task_results，同时返回结果供同步模式使用"""
    try:
        ...
        task_results[run_id] = {"status": "completed", "result": result}
        logger.info(f"Task {run_id} completed successfully")
        return result
    except asyncio.CancelledError:
        task_results[run_id] = {"status": "cancelled", "error": "Execution was cancelled"}
        logger.info(f"Task {run_id} was cancelled")
        raise
    except Exception as e:
        error_response = service.error_classifier.get_error_response(e, {"node_name": "run", "run_id": run_id})
        task_results[run_id] = {
            "status": "failed",
            "error": error_response.get("error_message", str(e)),
            "error_code": error_response.get("error_code", "UNKNOWN")
        }
        logger.error(f"Task {run_id} failed: {error_response.get('error_message', str(e))}")
        raise
...
async def http_run(request: Request) -> Dict[str, Any]:
    ...
    try:
        ...
        # 异步模式：立即返回run_id，后台继续执行，通过 /result/{run_id} 轮询结果
        # 默认同步模式（等待结果），部署环境使用 ?async_mode=true 避免网关超时
        async_mode = request.query_params.get("async_mode", "false").lower() == "true"

        # 初始化任务结果状态
        task_results[run_id] = {"status": "processing", "run_id": run_id}

        # 创建后台任务 - 任务与HTTP请求解耦，即使请求断开也会继续执行
        task = asyncio.create_task(_run_task_and_store(run_id, payload, ctx))
        service.running_tasks[run_id] = task

        if async_mode:
            # 异步模式：立即返回，客户端轮询 /result/{run_id} 获取结果
            logger.info(f"Async mode: returning immediately for run_id: {run_id}")
            return {
                "status": "processing",
                "run_id": run_id,
                "message": "Task started. Poll /result/{run_id} for results."
            }
        else:
            # 同步模式（兼容旧调用）：等待任务完成
            try:
...
async def get_result(run_id: str):
    """轮询获取任务执行结果"""
    if run_id in task_results:
        task_info = task_results[run_id]
        ...
        if status == "completed":
            ...
            return JSONResponse(
                ...
                content={
                    ...
                    "error_code": task_info.get("error_code", "UNKNOWN")
                }
            )
            ...
            return {
                "status": "processing",
                "run_id": run_id,
                "message": "Task is still running. Please retry later."
            }
    elif run_id in service.running_tasks:
        # 任务在运行中但结果还没存入 task_results
        task = service.running_tasks[run_id]
        if task.done():
            return {
                "status": "completed",
                "run_id": run_id,
                "message": "Task just completed. Please retry to get result."
            }
        else:
            return {
                "status": "processing",
...
async def cleanup_result(run_id: str):
    """清理已完成的任务结果（可选）"""
    if run_id in task_results:
        del task_results[run_id]
...
@app.post("/api/warmup_coze")
async def api_warmup_coze():
    """预热Coze工作流，防止冷启动超时"""
    from graphs.nodes.coze_search_node import _call_coze_api

    try:
        result = _call_coze_api(["warmup"], retry_on_cold_start=False)
        return {"status": "ok", "warmup_result_count": len(result)}
    except Exception as e:
        logger.warning("Coze warmup failed (expected if cold start): %s", e)
        return {"status": "warmup_sent", "message": "Warmup request sent, cold start may be in progress"}


@app.post("/api/direct_search")
async def api_direct_search(request: Request):
    """Coze搜索测试接口"""
    import asyncio
    from concurrent.futures import ThreadPoolExecutor
    from graphs.nodes.coze_search_node import coze_search_node
    from graphs.state import CozeSearchInput
    from coze_coding_utils.runtime_ctx.context import new_context, Context
    from langchain_core.runnables import RunnableConfig
    ...
    ctx = new_context(method="direct_search")
    ...
    def _run():
        return coze_search_node(input_data, config, runtime)
