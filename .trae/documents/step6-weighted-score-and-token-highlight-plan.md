...
- 已确认的产品决策：
  - 全部技术特征共同分配 100 分，商品总分为所有特征得分相加。
  - 待确认部分按“中文有效字符 / 英文有效单词”比例计分。
  - 若任一特征出现“明确不相同”，整个商品总分直接归零。
...
- `IP-protral/src/app/results/page.tsx`
  - 当前商品列表为卡片布局，单卡只展示商品图、商品名、商品总分和特征得分小计。
...
- 商品总分 = 所有特征得分相加；若任一特征明确不相同则商品总分为 0。
...
- 每个独立权利要求各自按 100 分计算。
- 商品总分取所有特征得分之和；任一特征明确不相同则商品总分归零。
